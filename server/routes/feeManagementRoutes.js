...
router.get('/:doctorId', async (req, res) => {
  try {
    ...
    const fees = feeConfigurations.map(config => ({
      ...
      channelPartnerFee: config.dispensaryId.channelPartnerFee || 0,
      onlineFee: config.dispensaryId.bookingCommission || 0,
...
router.put('/dispensary-fees/:dispensaryId', async (req, res) => {
  try {
    ...
    res.status(200).json({
      message: 'Dispensary global fees updated successfully',
...
router.post('/:doctorId', async (req, res) => {
  try {
    ...
    const { doctorId } = req.params;
    const { dispensaryId, doctorFee, dispensaryFee, onlineFee, channelPartnerFee, bookingCode, bookingVisibleDays } = req.body;
    
    console.log(`Creating fee for doctor ${doctorId}, dispensary ${dispensaryId}`);
    console.log('Fee data:', { doctorFee, dispensaryFee, onlineFee, channelPartnerFee: channelPartnerFee || 0, bookingCode, bookingVisibleDays });
    ...
    if (existingFee) {
      ...
      const responseData = {
        ...
        channelPartnerFee: populatedFee?.dispensaryId?.channelPartnerFee || 0,
        onlineFee: populatedFee?.dispensaryId?.bookingCommission || 0,
    ...
    const populatedFee = await DoctorDispensary.findById(newFeeConfig._id)
      .populate('doctorId', 'name specialization')
      .populate('dispensaryId', 'name address channelPartnerFee bookingCommission')
      .lean();
...
router.put('/:doctorId/:feeId', async (req, res) => {
  try {
    ...
    const { doctorFee, dispensaryFee, onlineFee, channelPartnerFee, bookingCode, bookingVisibleDays } = req.body;
    
    console.log(`Updating fee ${feeId} for doctor ${doctorId}`);
    console.log('New fee data:', { doctorFee, dispensaryFee, onlineFee, channelPartnerFee: channelPartnerFee || 0, bookingCode, bookingVisibleDays });
    ...
    const responseData = {
      ...
      channelPartnerFee: updatedFee?.dispensaryId?.channelPartnerFee || 0,
      onlineFee: updatedFee?.dispensaryId?.bookingCommission || 0,
...
router.get('/doctor-dispensaries/fees/:doctorId/:feeId', async (req, res) => {
  try {
    ...
    const responseData = {
      ...
      channelPartnerFee: fee.dispensaryId.channelPartnerFee || 0,
      onlineFee: fee.dispensaryId.bookingCommission || 0,
