...
const BookingForm = ({ initialDoctorId, initialDispensaryId, initialDate, showCalendar }: BookingFormProps) => {
  ...
  // Handle pay online flow - create booking and redirect to payment
  const handlePayOnline = async (feesObj?: any) => {
    if (!selectedDoctor || !selectedDispensary || !selectedDate || !name || !phone ||
      !availability?.available || !availability.slots?.length) {
      toast({
        title: 'Missing information',
    ...
    try {
      ...
      const paymentResponse = await api.post(`/payments/dialog-genie/create-intent/${bookingId}`, {
        customer: {
          name: name,
      ...
      const errorMessage = error.response?.data?.message || error.message || 'There was an error initiating payment. Please try again.';
      toast({
            ...
            <div className="space-y-6">
              {currentStep === 0 ? (
                ...
                <BookingStep2
                  ...
                  setEmail={setEmail}
                  isLoading={isLoading}
                  isPayOnlineLoading={isPayOnlineLoading}
                  onBack={() => setCurrentStep(0)}
                  onConfirm={(feesObj, paymentMethod) => handleBooking(feesObj, paymentMethod)}
                  onPayOnline={(feesObj) => handlePayOnline(feesObj)}
                  doctorId={selectedDoctor}
                  dispensaryId={selectedDispensary}
                />
              )}
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                ...
                                <div>
                                  ...
                                  <p className="text-sm"><strong>Status:</strong>
                                    <span className={`ml-1 px-2 py-1 rounded text-xs ${result.status === 'scheduled' ? 'bg-blue-100 text-blue-800' :
                        ...
                        <div>
                          ...
                          <div className="space-y-1 text-sm">
                            ...
                            <p><strong>Status:</strong>
                              <span className={`ml-1 px-2 py-1 rounded text-xs font-medium ${foundBooking.status === 'scheduled' ? 'bg-blue-100 text-blue-800' :
